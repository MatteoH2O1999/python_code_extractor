...
from dummy import dummy_dict_enum

CURRENT_DIRECTORY = os.path.abspath(os.path.dirname(__file__))
_POSSIBLE_SITE_PATHS.add(os.path.join(CURRENT_DIRECTORY, "dummy"))


def function_dict_enum():
    return dummy_dict_enum


def test_extract_dict_enum_function():
    with open(
        os.path.join(CURRENT_DIRECTORY, "expected_results", "function_dict_enum.json")
    ) as json_file:
        json_txt = json_file.read()
    expected_code = _ExtractedCode.from_string(json_txt)
    extracted_code = _ExtractedCode.from_string(extract_code(function_dict_enum))
...
def test_load_dict_enum_function():
    with open(
        os.path.join(CURRENT_DIRECTORY, "expected_results", "function_dict_enum.json")
    ) as json_file:
        json_txt = json_file.read()
    ...
    assert extracted_method.__name__ == "function_dict_enum"
    enum_eq(extracted_method(), dummy_dict_enum)
