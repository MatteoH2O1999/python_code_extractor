...
def test_extract_dict_function():
    with open(
        os.path.join(CURRENT_DIRECTORY, "expected_results", "function_dict.json")
    ) as json_file:
        json_txt = json_file.read()
    expected_code = _ExtractedCode.from_string(json_txt)
    extracted_code = _ExtractedCode.from_string(extract_code(function_dict))
...
def test_load_dict_function():
    ...
    assert extracted_method.__name__ == "function_dict"
    assert extracted_method() == dummy_dict
