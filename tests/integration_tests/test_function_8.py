...
def test_extract_external_function_dependency_function():
    with open(
        os.path.join(CURRENT_DIRECTORY, "expected_results", "function_8.json")
    ) as json_file:
        json_txt = json_file.read()
    expected_code = _ExtractedCode.from_string(json_txt)
    extracted_code = _ExtractedCode.from_string(extract_code(function_8))
...
def test_load_external_function_dependency_function():
    with open(
        os.path.join(CURRENT_DIRECTORY, "expected_results", "function_8.json")
    ) as json_file:
        json_txt = json_file.read()
    ...
    assert extracted_method.__name__ == "function_8"
    assert extracted_method() == (random, mean)
